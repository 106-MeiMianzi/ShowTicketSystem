...
若已完成 Redis 相关改造，只需**最少**重测以下接口即可确认行为正确：

| 序号 | 接口 | 用例 | 目的 |
|------|------|------|------|
| R1 | `POST /api/user/logout` | 11.1 + **11.2** | 确认登出成功，且登出后原 Token 失效（11.2 为新增） |
| R2 | `GET /api/user/current` | 4.1、4.2 | 登出后用原 Token 调一次 → 应 401（与 11.2 一起测） |
| R3 | `GET /api/show/home` | 12.1 | 确认首页仍返回演出列表（含按 IP 推荐逻辑） |
| R4 | `POST /api/order/create` | 16.1 | 确认下单成功（Redis 库存 + 分布式锁） |

**无需新增接口**。热门排行榜（getPopularShows）目前仅内部使用，未对外暴露；若需要可后续加 `GET /api/show/popular?limit=10`。
...
- ✅ 个人开发者可用，无需企业资质
- ✅ 需要先关注微信测试号才能扫码登录
- ✅ 完整流程：获取场景值 → 获取二维码 → 扫码确认 → 查询状态 → 获取Token

**前置准备**：
1. 确保已关注微信测试号（用微信扫描测试号二维码）
2. 确保微信测试号已配置接口URL：`https://146ebb56.r11.vip.cpolar.cn/wxverify`
3. 确保内网穿透正在运行（cpolar）

**完整URL**：`https://146ebb56.r11.vip.cpolar.cn`（使用内网穿透公网地址）

---

##### 2.5.1. 获取场景值（sceneId）

**接口**：`GET /wxlogin/scene-id`  
**请求方式**：`GET`  
**完整URL**：`https://146ebb56.r11.vip.cpolar.cn/wxlogin/scene-id`  
**需要Token**：❌ 否

| 序号 | 用例描述 | 请求参数 | 目标分类 | 测试标签 | 预期结果 | 测试状态 |
|------|---------|---------|---------|---------|---------|---------|
| WX1.1 | 获取场景值成功 | 无参数 | 正向 | 仅传必要字段、语义合法 | 返回200，响应体为随机字符串（如：`ecf31658-dc63-4167-9342-f5f29437980e`） | ⬜ |

**测试提示**：
- ✅ 成功后，**务必保存返回的 sceneId**，后续步骤需要使用
- ✅ 每个 sceneId 只能使用一次，获取 Token 后就不能再用了

---

##### 2.5.2. 获取登录二维码

**接口**：`GET /wxlogin/qrcode`  
**请求方式**：`GET`  
**完整URL**：`https://146ebb56.r11.vip.cpolar.cn/wxlogin/qrcode?sceneId=ecf31658-dc63-4167-9342-f5f29437980e`  
**需要Token**：❌ 否

| 序号 | 用例描述 | 请求参数 | 目标分类 | 测试标签 | 预期结果 | 测试状态 |
|------|---------|---------|---------|---------|---------|---------|
| WX2.1 | 获取二维码成功 | `sceneId: ecf31658-dc63-4167-9342-f5f29437980e`（步骤1获取的sceneId） | 正向 | 仅传必要字段、语义合法 | 返回200，响应体为微信二维码图片URL（如：`https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=xxxx`） | ⬜ |
| WX2.2 | sceneId为空 | `sceneId: ""` | 负向 | 缺失必填字段 | 返回200，自动生成新的sceneId并返回二维码URL | ⬜ |
| WX2.3 | sceneId不存在 | `sceneId: invalid_scene_id` | 负向 | 无效值 | 返回200，自动生成新的sceneId并返回二维码URL | ⬜ |

**测试提示**：
- ✅ 复制返回的二维码URL，在浏览器中打开即可看到二维码
- ✅ 二维码有时效性，建议尽快扫码
...
**接口**：`GET /wxlogin/status`  
**请求方式**：`GET`  
**完整URL**：`https://146ebb56.r11.vip.cpolar.cn/wxlogin/status?sceneId=ecf31658-dc63-4167-9342-f5f29437980e`  
**需要Token**：❌ 否

| 序号 | 用例描述 | 请求参数 | 目标分类 | 测试标签 | 预期结果 | 测试状态 |
|------|---------|---------|---------|---------|---------|---------|
| WX3.1 | 查询登录状态（未扫码） | `sceneId: ecf31658-dc63-4167-9342-f5f29437980e` | 正向 | 语义合法 | 返回200，响应体为 `"fail"`（表示未登录） | ⬜ |
| WX3.2 | 查询登录状态（已扫码确认） | `sceneId: ecf31658-dc63-4167-9342-f5f29437980e`（用户已扫码并确认） | 正向 | 语义合法 | 返回200，响应体为 `"success"`（表示登录成功） | ⬜ |
| WX3.3 | sceneId为空 | `sceneId: ""` | 负向 | 缺失必填字段 | 返回200，响应体为 `"fail"` | ⬜ |
| WX3.4 | sceneId不存在 | `sceneId: invalid_scene_id` | 负向 | 无效值 | 返回200，响应体为 `"fail"` | ⬜ |
...
**接口**：`GET /api/user/wechat-token`  
**请求方式**：`GET`  
**完整URL**：`https://146ebb56.r11.vip.cpolar.cn/api/user/wechat-token?sceneId=ecf31658-dc63-4167-9342-f5f29437980e`  
**需要Token**：❌ 否（此接口不需要Token，因为用户刚通过微信登录）

| 序号 | 用例描述 | 请求参数 | 目标分类 | 测试标签 | 预期结果 | 测试状态 |
|------|---------|---------|---------|---------|---------|---------|
| WX4.1 | 获取Token成功 | `sceneId: ecf31658-dc63-4167-9342-f5f29437980e`（步骤3已返回success） | 正向 | 仅传必要字段、语义合法 | 返回200，JSON格式：`{"success": true, "message": "登录成功", "data": {"token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...", "userId": 1, "username": "wx_abc1234", "role": 1, "message": "微信登录成功！欢迎，wx_abc1234"}}` | ⬜ |
| WX4.2 | sceneId无效或已使用 | `sceneId: invalid_scene_id` | 负向 | 无效值 | 返回400，`{"success": false, "message": "未找到登录信息，请重新扫码"}` | ⬜ |
| WX4.3 | sceneId为空 | `sceneId: ""` | 负向 | 缺失必填字段 | 返回400，`{"success": false, "message": "未找到登录信息，请重新扫码"}` | ⬜ |
| WX4.4 | 重复获取Token | `sceneId: ecf31658-dc63-4167-9342-f5f29437980e`（已使用过的sceneId） | 负向 | 无效值 | 返回400，`{"success": false, "message": "未找到登录信息，请重新扫码"}`（Token只能获取一次） | ⬜ |

**测试提示**：
- ✅ **重要**：此接口只能调用一次，获取 Token 后 sceneId 会被删除
- ✅ 成功后，**务必保存返回的 token**，用于后续需要认证的接口
- ✅ 如果返回"未找到登录信息"，需要重新获取 sceneId 和二维码
- ✅ Token 格式与普通登录接口返回的格式一致，可以直接使用

**完整测试流程示例**：

1. **获取场景值**：
   ```
   GET https://146ebb56.r11.vip.cpolar.cn/wxlogin/scene-id
   响应：ecf31658-dc63-4167-9342-f5f29437980e
   ```

2. **获取二维码**：
   ```
   GET https://146ebb56.r11.vip.cpolar.cn/wxlogin/qrcode?sceneId=ecf31658-dc63-4167-9342-f5f29437980e
   响应：https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=xxxx
   ```
...
4. **查询登录状态**（轮询，每2-3秒一次）：
   ```
   GET https://146ebb56.r11.vip.cpolar.cn/wxlogin/status?sceneId=ecf31658-dc63-4167-9342-f5f29437980e
   响应：success（表示登录成功）
   ```

5. **获取Token**：
   ```
   GET https://146ebb56.r11.vip.cpolar.cn/api/user/wechat-token?sceneId=ecf31658-dc63-4167-9342-f5f29437980e
   响应：{"success": true, "message": "登录成功", "data": {"token": "...", "userId": 1, ...}}
   ```

6. **使用Token访问需要认证的接口**：
   ```
   GET https://146ebb56.r11.vip.cpolar.cn/api/user/current
   Headers: Authorization: Bearer {token}
   ```

**注意事项**：
- ⚠️ **内网穿透地址**：如果 cpolar 重启，地址可能会变化，需要重新配置微信测试号的 URL
- ⚠️ **测试号限制**：只有关注了测试号的微信才能扫码登录
- ⚠️ **sceneId一次性**：每个 sceneId 只能使用一次，获取 Token 后就不能再用了
- ⚠️ **二维码时效性**：二维码有时效性，建议在获取后尽快扫码
...
**需要Token**：建议携带（携带则将该 Token 加入黑名单，登出后该 Token 不可再用）

**说明（Redis 优化后）**：登出会将当前 Token 加入 Redis 黑名单，登出后使用同一 Token 访问需认证接口会返回 401。

| 序号 | 用例描述 | 请求头 | 目标分类 | 测试标签 | 预期结果 | 测试状态 |
|------|---------|-------|---------|---------|---------|---------|
| 11.1 | 退出登录 | `Authorization: Bearer {{token}}` | 正向 | 语义合法 | 返回200，message 含「退出登录成功」且 success 为 true（全文可能含「Token已失效」） | ⬜ |
| 11.2 | 登出后使用原 Token 访问 | 先执行 11.1，再用**同一 Token** 请求 `GET /api/user/current` | 安全性 | 其他 | 返回 401，`{"error":"未授权：请先登录"}`（验证黑名单生效） | ⬜ |
...
**Redis 优化后说明**：
- 创建订单接口增加了**限流**（同一用户每分钟最多 5 次）：超限时返回 **429**，body 含「请求过于频繁」。
- 增加了**防重复下单**（分布式锁）：同一用户对同一演出短时间内重复提交可能返回 **400**，错误信息含「请勿重复提交订单」。
...
**重要提示**：
- ⚠️ **测试前准备**：
  1. 确保已配置支付宝沙箱环境（APPID、私钥、公钥）
  2. 确保内网穿透工具（cpolar）正在运行
  3. 确保回调地址已配置：`https://24920b45.r29.cpolar.top/api/payment/notify`
  4. 确保应用已启动并运行在8080端口
- ✅ **支付流程**：
  1. 先创建订单（status=1待支付）
  2. 调用创建支付接口，获取支付页面HTML（payHtml）
  3. 将payHtml在浏览器中打开，使用支付宝沙箱账号扫码支付
  4. 支付完成后，支付宝会自动回调 `/api/payment/notify` 接口
  5. 检查订单状态是否更新为"已支付"（status=2）
  6. 检查是否收到邮件通知
- 📝 **定时任务说明**：
  - **掉单补偿**：每5分钟自动执行，查询待支付订单的支付宝状态并补偿
  - **超时关单**：每10分钟自动执行，关闭超过30分钟未支付的订单
  - 这两个功能是自动运行的，无需手动测试
...
**请求方式**：`POST`  
**完整URL**：`http://localhost:8080/api/payment/create?orderId=1`  
**需要Token**：✅ 是  
**Content-Type**：`application/x-www-form-urlencoded` 或 `application/json`

**接口说明**：
- ✅ 调用支付宝SDK创建支付订单
- ✅ 返回支付页面HTML（payHtml），前端可以直接展示给用户
- ✅ 用户扫码或输入密码完成支付
- ✅ 支付完成后，支付宝会自动回调 `/api/payment/notify` 接口

**请求参数说明**：
- `orderId`（必填，URL参数或Query参数）：订单ID，必须是待支付状态（status=1）的订单

**Apifox设置步骤**：
1. 请求方式选择：`POST`
2. URL：`http://localhost:8080/api/payment/create?orderId=1`（orderId替换为实际的订单ID）
3. Headers：添加 `Authorization: Bearer {{token}}`
4. Body：无需请求体（orderId通过URL参数传递）
...
- 成功响应（200）：
  ```json
  {
    ...
    "message": "支付订单创建成功",
    "data": {
      "payHtml": "<form name='punchout_form' method='post' action='https://openapi.alipaydev.com/gateway.do?...'>...</form>",
      "orderNo": "ORDER20240220123456789012345678",
      "message": "支付订单创建成功，请完成支付"
    }
  }
  ```
- `payHtml`：支付页面HTML，可以直接在浏览器中打开或嵌入到iframe中展示
- `orderNo`：商户订单号

| 序号 | 用例描述 | 请求头 | 请求参数 | 目标分类 | 测试标签 | 预期结果 | 测试状态 |
|------|---------|-------|---------|---------|---------|---------|---------|
| 22.1 | 创建支付订单成功 | `Authorization: Bearer {{token}}` | `orderId: 1`（待支付的订单ID，status=1） | 正向 | 仅传必要字段、语义合法 | 返回200，`{"success": true, "message": "支付订单创建成功", "data": {"payHtml": "...", "orderNo": "...", "message": "..."}}` | ⬜ |
| 22.2 | 未登录 | 无Authorization头 | `orderId: 1` | 安全性 | 缺失必填字段 | 返回401，`{"success": false, "message": "请先登录"}` | ⬜ |
| 22.3 | 订单不存在 | `Authorization: Bearer {{token}}` | `orderId: 999999` | 负向 | 无效值 | 返回400，`{"success": false, "message": "创建支付订单失败！订单不存在、不属于您、或状态不可支付。"}` | ⬜ |
| 22.4 | 订单不属于当前用户 | `Authorization: Bearer {{token}}` | `orderId: 其他用户的订单ID` | 安全性 | 无效值 | 返回400，`{"success": false, "message": "创建支付订单失败！订单不存在、不属于您、或状态不可支付。"}` | ⬜ |
| 22.5 | 订单状态不可支付（已支付） | `Authorization: Bearer {{token}}` | `orderId: X`（status=2已支付的订单） | 负向 | 无效值 | 返回400，`{"success": false, "message": "创建支付订单失败！订单不存在、不属于您、或状态不可支付。"}` | ⬜ |
| 22.6 | 订单状态不可支付（已取消） | `Authorization: Bearer {{token}}` | `orderId: X`（status=3已取消的订单） | 负向 | 无效值 | 返回400，`{"success": false, "message": "创建支付订单失败！订单不存在、不属于您、或状态不可支付。"}` | ⬜ |
| 22.7 | orderId为空 | `Authorization: Bearer {{token}}` | `orderId: `（空值） | 负向 | 缺失必填字段 | 返回400，参数错误 | ⬜ |
| 22.8 | orderId为0 | `Authorization: Bearer {{token}}` | `orderId: 0` | 边界值 | 无效值 | 返回400，`{"success": false, "message": "创建支付订单失败！订单不存在、不属于您、或状态不可支付。"}` | ⬜ |
| 22.9 | orderId为负数 | `Authorization: Bearer {{token}}` | `orderId: -1` | 边界值 | 无效值 | 返回400，`{"success": false, "message": "创建支付订单失败！订单不存在、不属于您、或状态不可支付。"}` | ⬜ |

**测试提示**：
- ✅ **用例22.1测试流程**：
  1. 先创建一个订单（调用 `POST /api/order/create`），确保订单状态为待支付（status=1）
  2. 调用此接口创建支付订单
  3. 复制返回的 `payHtml` 内容
  4. 创建一个HTML文件，将 `payHtml` 内容粘贴进去
  5. 在浏览器中打开HTML文件，会看到支付宝支付页面
  6. 使用支付宝沙箱账号扫码或输入密码完成支付
  7. 支付完成后，检查订单状态是否更新为"已支付"（status=2）
  8. 检查是否收到邮件通知
- ⚠️ **注意**：`payHtml` 是完整的HTML表单，可以直接在浏览器中打开
- ⚠️ **支付宝沙箱账号**：需要在支付宝开放平台沙箱环境中查看沙箱账号信息
...
**请求方式**：`POST`  
**完整URL**：`http://localhost:8080/api/payment/notify` 或 `https://24920b45.r29.cpolar.top/api/payment/notify`（公网地址）  
**需要Token**：❌ 否（此接口不需要Token，由支付宝服务器调用）

**接口说明**：
- ✅ 此接口由支付宝服务器在用户支付完成后自动调用
- ✅ 接口会验证签名，确保回调来自支付宝
- ✅ 处理成功后更新订单状态为"已支付"（status=2）
- ✅ 保存支付宝交易号
- ✅ 发送支付成功邮件通知
...
- `trade_no`：支付宝交易号（必填）
- `trade_status`：交易状态（必填）
  - `TRADE_SUCCESS`：交易成功（即时到账）
  - `TRADE_FINISHED`：交易完成（担保交易）
  - `WAIT_BUYER_PAY`：等待买家付款
  - `TRADE_CLOSED`：交易关闭
- `sign`：签名（必填，用于验证回调真实性）
- 其他参数：支付宝可能传递的其他参数

**Apifox设置步骤（模拟支付宝回调）**：
1. 请求方式选择：`POST`
2. URL：`http://localhost:8080/api/payment/notify` 或 `https://24920b45.r29.cpolar.top/api/payment/notify`
3. Body类型选择：`form-data` 或 `x-www-form-urlencoded`
4. 添加参数（模拟支付成功回调）：
   - `out_trade_no`：商户订单号（从创建支付订单接口获取的orderNo）
   - `trade_no`：支付宝交易号（模拟值，如：`2024022022001234567890123456`）
   - `trade_status`：`TRADE_SUCCESS`（支付成功）
   - `sign`：签名（模拟值，实际测试时签名验证会失败，但可以测试其他逻辑）
   - `sign_type`：`RSA2`
   - `charset`：`UTF-8`
   - `timestamp`：`2024-02-20 21:00:00`
   - `version`：`1.0`
   - `app_id`：`9021000161677599`（你的APPID）

**响应说明**：
- 成功响应：返回字符串 `"success"`（支付宝收到此响应后不会再回调）
- 失败响应：返回字符串 `"failure"`（支付宝会继续回调，直到收到 `"success"`）

| 序号 | 用例描述 | 请求方式 | 请求参数（Body） | 目标分类 | 测试标签 | 预期结果 | 测试状态 |
|------|---------|---------|-----------------|---------|---------|---------|---------|
| 23.1 | 支付成功回调（TRADE_SUCCESS） | `POST` | `out_trade_no: ORDER20240220123456789012345678`<br>`trade_no: 2024022022001234567890123456`<br>`trade_status: TRADE_SUCCESS`<br>`sign: ...`<br>`其他参数...` | 正向 | 语义合法 | 返回200，响应体为字符串 `"success"`，订单状态更新为"已支付"（status=2），收到邮件通知 | ⬜ |
| 23.2 | 支付成功回调（TRADE_FINISHED） | `POST` | `out_trade_no: ORDER20240220123456789012345678`<br>`trade_no: 2024022022001234567890123456`<br>`trade_status: TRADE_FINISHED`<br>`sign: ...`<br>`其他参数...` | 正向 | 语义合法 | 返回200，响应体为字符串 `"success"`，订单状态更新为"已支付"（status=2），收到邮件通知 | ⬜ |
| 23.3 | 商户订单号为空 | `POST` | `out_trade_no: ""`<br>`trade_status: TRADE_SUCCESS`<br>`其他参数...` | 负向 | 缺失必填字段 | 返回200，响应体为字符串 `"failure"` | ⬜ |
| 23.4 | 订单不存在 | `POST` | `out_trade_no: INVALID_ORDER_NO`<br>`trade_status: TRADE_SUCCESS`<br>`其他参数...` | 负向 | 无效值 | 返回200，响应体为字符串 `"failure"` | ⬜ |
| 23.5 | 订单已支付（重复回调） | `POST` | `out_trade_no: ORDER20240220123456789012345678`（订单已支付）<br>`trade_status: TRADE_SUCCESS`<br>`其他参数...` | 正向 | 其他正向 | 返回200，响应体为字符串 `"success"`（防止重复处理） | ⬜ |
| 23.6 | 签名验证失败 | `POST` | `out_trade_no: ORDER20240220123456789012345678`<br>`trade_status: TRADE_SUCCESS`<br>`sign: INVALID_SIGN`<br>`其他参数...` | 安全性 | 无效值 | 返回200，响应体为字符串 `"failure"`（签名验证失败） | ⬜ |
| 23.7 | 交易状态为等待付款 | `POST` | `out_trade_no: ORDER20240220123456789012345678`<br>`trade_status: WAIT_BUYER_PAY`<br>`其他参数...` | 负向 | 无效值 | 返回200，响应体为字符串 `"failure"`（等待付款状态不处理） | ⬜ |
| 23.8 | 交易状态为交易关闭 | `POST` | `out_trade_no: ORDER20240220123456789012345678`<br>`trade_status: TRADE_CLOSED`<br>`其他参数...` | 负向 | 无效值 | 返回200，响应体为字符串 `"failure"`（交易关闭状态不处理） | ⬜ |
| 23.9 | GET请求（错误方法） | `GET` | 无参数 | 负向 | 格式错误 | 返回405，`Method Not Allowed`（此接口只接受POST请求） | ⬜ |

**测试提示**：
- ⚠️ **重要**：此接口一般由支付宝服务器自动调用，手动测试主要用于验证接口逻辑
- ⚠️ **签名验证**：实际回调时，支付宝会传递正确的签名。手动测试时签名验证可能会失败，这是正常的
- ✅ **用例23.1和23.2测试流程**：
  1. 先创建一个订单并调用创建支付接口
  2. 使用真实的商户订单号（out_trade_no）模拟回调
  3. 设置 `trade_status` 为 `TRADE_SUCCESS` 或 `TRADE_FINISHED`
  4. 发送POST请求
  5. 检查响应是否为 `"success"`
  6. 查询订单状态，确认是否更新为"已支付"（status=2）
  7. 检查是否收到邮件通知
- ✅ **用例23.5**：测试重复回调处理，确保不会重复更新订单状态
- ✅ **用例23.9**：验证接口只接受POST请求，GET请求会返回405错误

**实际支付测试流程**：
1. 创建订单（`POST /api/order/create`）
2. 创建支付订单（`POST /api/payment/create`），获取 `payHtml`
3. 将 `payHtml` 在浏览器中打开，使用支付宝沙箱账号支付
4. 支付完成后，支付宝会自动调用此回调接口
5. 检查订单状态和邮件通知
...
**说明**：
- ✅ 这两个功能是**定时任务**，自动运行，无需手动测试
- ✅ **掉单补偿**：每5分钟执行一次，查询待支付订单的支付宝状态并补偿
- ✅ **超时关单**：每10分钟执行一次，关闭超过30分钟未支付的订单
- ✅ 可以通过查看应用日志来确认定时任务是否正常运行
...
- **功能说明**：
  - 查询30分钟前创建的待支付订单
  - 调用支付宝API查询订单状态
  - 如果订单在支付宝中已支付，但系统状态还是待支付，则更新订单状态
  - 保存支付宝交易号
  - 发送邮件通知
- **测试方法**：
  - 创建订单但不支付
...
- **功能说明**：
  - 查询30分钟前创建的待支付订单
  - 先查询支付宝订单状态
  - 如果订单在支付宝中未支付或已关闭，则关闭订单并恢复库存
  - 如果订单在支付宝中已支付，则执行掉单补偿逻辑
- **测试方法**：
  - 创建订单但不支付
  - 等待30分钟以上
  - 检查订单状态是否自动更新为"已取消"（status=3）
  - 检查演出库存是否恢复
  - 查看应用日志，确认定时任务执行情况
...
| 第三阶段 | 演出查询 | 20 | | | |
| 第四阶段 | 订单管理 | 30 | | | |
| 第五阶段 | 支付模块（支付宝沙箱） | 18 | | | 包含创建支付订单、支付回调、定时任务说明 |
| 第六阶段 | 管理端接口 | 50 | | | |
| **总计** | | **171** | | | |
